                ...
                <div class="panel-heading">
                	<div class="row">
                        ...
                        <div class="col-lg-2 col-md-2 col-sm-4 col-xs-6" align="right">
                            <button type="button" name="add" id="add_button" class="btn btn-success btn-xs">Create Outlet Sale Bill</button>    	
                        </div>
                    </div>
                </div>
    		...
    		<form method="post" id="order_form">
    			<div class="modal-content">
    				<div class="modal-header">
						...
						<h4 class="modal-title"><i class="fa fa-plus"></i> Create Outlet Sales Order</h4>
    				</div>
							...
							<div class="col-md-6">
								<div class="form-group">
									...
									<select name="outlet_id" id="outlet_id" class="form-control" required>
										<?php echo fill_outlet_list($connect);?>										
									</select>
									<input type="text" name="outlet_name" id="outlet_name" class="form-control" style="visibility:hidden;"/>
								<!--	<button name="Addoutlet" id="Addoutlet"  class="form-control" style="visibility:hidden;">Addoutlet</button>	-->
								</div>
							</div>
							<div class="col-md-6">
								<div class="form-group">
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
    	...
    	var orderdataTable = $('#order_data').DataTable({
			...
			"searching":true,
			"ajax":{
				url:"outlet_stock_allocation_fetch.php",
				type:"POST"
			},
			"dom":'lBfrtrip',
				"columnDefs":[
		...
		$('#outlet_id').change(function(){
			...
			{
				//$('#outlet_name').show();
				document.getElementById('outlet_name').style.visibility="visible";
				document.getElementById('Addoutlet').style.visibility="visible";
			}
			else
			{
				//$('#outlet_name').hide();
				document.getElementById('outlet_name').style.visibility="hidden";
				document.getElementById('Addoutlet').style.visibility="hidden";
			}
		});
		$('#Addoutlet').click(function(){
			var outlet_name = $('#outlet_name').val();
			var outlet_id = $('#outlet_id').val();
			var btn_action = "Addoutlet";
			if(outlet_id=="other" && outlet_name!='')
			{
				if(confirm("Are you sure you want to Add outlet?"))
				{
					$.ajax({
						url:"outlet_stock_allocation_action.php",
						method:"POST",
						data:{outlet_id:outlet_id,outlet_name:outlet_name, btn_action:btn_action},
						success:function(data)
						{
							alert("Outlet Has been Added");							
						}
					});
				}
				else
		...
		$('#add_button').click(function(){
			...
			$('#order_form')[0].reset();
			$('.modal-title').html("<i class='fa fa-plus'></i> Create Outlet Order");
			$('#action').val('Add');
			$('#btn_action').val('Add');
