...
$output = array();
	...
	$query .= "
		...
		where customer_details.customer_id = inventory_order.customer_id and inventory_order.inventory_order_id = inventory_order_product.inventory_order_id
			  and inventory_order.bill_type !='BroughtForward'
	";
...
foreach($result as $row)
{
	...
	$sub_array[] = $row['GSTIN'];
	$sub_array[] = $row['invoice_no'];//$invoice_id;//$row['inventory_order_id'];
...
//	$sub_query = '';
	...
	product.tax_status,product.SGST as sgst_tax,product.CGST as cgst_tax, product.product_unit as product_unit
	from
	...
	if($filtered_rows1>0)
	{
		foreach($result1 as $row1)
		{
			...
			if($row1['sgst_tax']==0)
			{
				$value0 += ($weight*$row1['quantity']*$row1['price']);
				$SGST0 += ($weight*$row1['quantity']*$row1['price']*$row1['sgst_tax'])/100;
				$CGST0 += ($weight*$row1['quantity']*$row1['price']*$row1['cgst_tax'])/100;
			}
			if($row1['sgst_tax']==2.5)
			{
				$value2 += ($weight*$row1['quantity']*$row1['price']);
				$SGST2 += ($weight*$row1['quantity']*$row1['price']*$row1['sgst_tax'])/100;
				$CGST2 += ($weight*$row1['quantity']*$row1['price']*$row1['cgst_tax'])/100;
			}
			else if($row1['sgst_tax']==6)
			{
				$value6 += ($weight*$row1['quantity']*$row1['price']);
				$SGST6 += ($weight*$row1['quantity']*$row1['price']*$row1['sgst_tax'])/100;
				$CGST6 += ($weight*$row1['quantity']*$row1['price']*$row1['cgst_tax'])/100;
			}	
			else if($row1['sgst_tax']==9)
			{
				$value9 += ($weight*$row1['quantity']*$row1['price']);
				$SGST9 += ($weight*$row1['quantity']*$row1['price']*$row1['sgst_tax'])/100;
				$CGST9 += ($weight*$row1['quantity']*$row1['price']*$row1['cgst_tax'])/100;
			}	
			else if($row1['sgst_tax']==14)
			{
				$value14 += ($weight*$row1['quantity']*$row1['price']);
				$SGST14 += ($weight*$row1['quantity']*$row1['price']*$row1['sgst_tax'])/100;	
				$CGST14 += ($weight*$row1['quantity']*$row1['price']*$row1['cgst_tax'])/100;	
			}
		}
		$GST += $SGST0+$SGST2+$SGST6+$SGST9+$SGST14+$CGST0+$CGST2+$CGST6+$CGST9+$CGST14;
		$value = $value0+$value2+$value6+$value9+$value14;
	}
	$sub_array[] = round($value0);
	$sub_array[] = round($value2);
	$sub_array[] = round($value6);
	$sub_array[] = round($value9);
	$sub_array[] = round($value14);
	$sub_array[] = round($value);
	$sub_array[] = round($SGST2);
	$sub_array[] = round($SGST6);
	$sub_array[] = round($SGST9);
	$sub_array[] = round($SGST14);
	$sub_array[] = round($SGST2);
	$sub_array[] = round($SGST6);
	$sub_array[] = round($SGST9);
	$sub_array[] = round($SGST14);
	$sub_array[] = round($GST);
	$sub_array[] = round($value+$GST);
