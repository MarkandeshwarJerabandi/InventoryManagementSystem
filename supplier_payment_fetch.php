...
/*	$status = '';
	...
	$sub_array[] = '<a href="view_supplier_payment_report.php?pdf=1&payment_id='.$row["payment_id"].'" class="btn btn-info btn-xs">View Payment Report</a>';
	if($row['balance']>0)
		$sub_array[] = '<a href="make_supplier_payment.php?payment_id='.$row["payment_id"].'" class="btn btn-info btn-xs">Make Payment</a>';
	else
		$sub_array[] = '<a href="supplier_payment.php" class="btn btn-info btn-xs">Make Payment</a>';
//	$sub_array[] = '<a href="make_supplier_payment.php?payment_id='.$row["payment_id"].'" class="btn btn-info btn-xs">Update Payment</a>';
	$data[] = $sub_array;
}

function get_total_all_records($connect)
{
	$statement = $connect->prepare("SELECT * FROM supplier_payment");
	$statement->execute();
