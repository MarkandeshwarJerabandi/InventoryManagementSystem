<?php
//header.php
?>
<!DOCTYPE html>
<html>
	<head>
		<title>Purchase, Sales and Stock Management System</title>
		<script src="js/jquery-1.10.2.min.js"></script>
		<link rel="stylesheet" href="css/bootstrap.min.css" />
		<script src="js/jquery.dataTables.min.js"></script>
		<script src="js/dataTables.bootstrap.min.js"></script>		
		<link rel="stylesheet" href="css/dataTables.bootstrap.min.css" />
		<script src="js/bootstrap.min.js"></script>
	</head>
	<body>
		<br />
		<div class="container">
		<!--	<h2 align="center">Purchase, Sales and Stock Management System</h2>	-->

			<nav class="navbar navbar-inverse">
				<div class="container-fluid">
					<div class="navbar-header">
						<a href="index.php" class="navbar-brand">Home</a>
					</div>
					<ul class="nav navbar-nav">
					<?php
					if($_SESSION['type'] == 'master')
					{
					?>
						<li><a href="customer.php">Customers</a></li>
						<li><a href="supplier.php">Farmers</a></li>
					<!--	<li><a href="category.php">Product Category</a></li>
						<li><a href="brand.php">Brand</a></li>	-->
						<li><a href="product.php">Products</a></li>
						<li><a href="purchase.php">Purchases</a></li>
						<li><a href="stock_entry.php">Stock Entry</a></li>
						<li><a href="sales.php">Sales</a></li>
				<!--		<li><a href="quotation.php">Quotation</a></li>
						<li><a href="display.php">Display</a></li>		-->
						<li><a href="supplier_payment.php">Suppliers Payment</a></li>
						<li><a href="customer_payment.php">Customers Payment</a></li>
						<li><a href="reports.php">Reports</a></li>
						<li><a href="backup/index.php">Create Backup</a></li>
					<!--	<li><a href="user.php">User</a></li>	-->
					<?php
					}
					else
					{
					?>
						<li><a href="sales.php">Sales</a></li>
					</ul>
					<?php
					}
					?>
					<ul class="nav navbar-nav navbar-right">
						<li class="dropdown">
							<a href="#" class="dropdown-toggle" data-toggle="dropdown"><span class="label label-pill label-danger count"></span> <?php echo $_SESSION["user_name"]; ?></a>
							<ul class="dropdown-menu">
								<li><a href="profile.php">Profile</a></li>
								<li><a href="logout.php">Logout</a></li>
							</ul>
						</li>
					</ul>

				</div>
			</nav>
		
			